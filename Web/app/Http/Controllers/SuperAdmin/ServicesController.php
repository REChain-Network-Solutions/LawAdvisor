...
use App\Exports\SuperAdmin\LawyerServicesExport;
use App\Http\Controllers\Controller;
use App\Http\Requests\SuperAdmin\Services\CreateRequest;
use App\Http\Requests\ImportRequest;
use App\Imports\SuperAdmin\LawyerServicesImport;
...
class ServicesController extends Controller
{
  ...
  public function __construct()
  {
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new LawyerServicesExport($services), $filename);
    }
    /********* Import CSV And Excel  **********/
    public function import(ImportRequest $request)
    {
        $file = $request->file('file');
        Excel::import(new LawyerServicesImport, $file);
     ...
     public function bulkActionServices(Request $request, $type)
     {
         if ($type == 'approve') {
             ...
             foreach ($request->selected_ids as $userId) {
                 $lawyer = Service::where('id', $userId)->first();
                 $this->destroy($lawyer);
             }
         } elseif ($type == 'feature') {
             Service::whereIn('id', $request->selected_ids)->update([
