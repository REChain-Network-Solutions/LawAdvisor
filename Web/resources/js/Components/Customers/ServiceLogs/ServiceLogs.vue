        ...
        <div class="row">
            <div class="col-12">
                <h2 class="fs-2 text-center">
                    <span class="fw-normal">{{ __("Hello") }} {{ $page.props.auth.user.customer.first_name }} {{ $page.props.auth.user.customer.last_name }} | </span>
                    <span class="fw-bold">{{ __("my services") }}</span>
                </h2>
            ...
            <div class="row">
                <div class="col-md-3 p-4">
                    <div class="nav flex-column nav-pills account-tabs" id="v-pills-tab" role="tablist"
                        ...
                        <li class="nav-item mb-3" role="presentation" @click="() => (currentTab = appIndex)"
                            ...
                            <button class="nav-link w-100 text-dark" :class="{ active: currentTab === appIndex }"
                                :id="'pills-service-tab-' + appIndex" data-bs-toggle="pill"
                ...
                <div class="col-md-9 border-start border-dark">
                    <div class="tab-content w-100 p-4" id="v-pills-tabContent">
                        <div v-for="(status, index) in service_statuses" :key="index" class="tab-pane fade"
                            :class="{ 'active show': currentTab === index }" :id="'pills-home-' + index" role="tabpanel"
                            :aria-labelledby="'pills-service-tab-' + index">
                            <div class="row">
                                <div class="col-md-12">
                                    <div v-if="currentTab === index">
                                            ...
                                            <div class="d-md-flex align-items-center justify-content-end mb-3">
                                                ...
                                                <div class="form-group me-2 mb-3 mb-md-0">
                                                    <select  v-model="filter.column"
                                                        ...
                                                        <option value="" selected>{{ __('select') }} {{ __('filter') }}
                                                        </option>
                                                        <option v-for="col in this.columns" :key="col.id"
                                                            :value="col.value">{{ col.name }}</option>
                                                    </select>
                                                </div>
                                        ...
                                        <customer-service-list-card
                                            :services="services"></customer-service-list-card>
                                        <div class="col-12">
...
import CustomerServiceListCard from "@/Components/Customers/ServiceLogs/CustomerServiceListCard.vue";
...
export default defineComponent({
    ...
    async created() {
        ...
        await this.getServiceFilterData(false);
    },
    methods: {
        async getPaginatedData(loading_more = false, status_code) {
            ...
            this.filter.status_code = this.status_code
            await this.getServiceFilterData(loading_more)
        },
        getServiceFilterData(loading_more) {
            this.fetching = true
            axios.post(this.route('getApiFilterServiceLogs'), this.filter).then(res => {
